...
class LetterEnvelope:
    ...
    def ap_start(self):
        ...
        p2 = sympy.randprime(2**(KEY_SIZE-1),(2**KEY_SIZE)-1)
        q2 = sympy.randprime(2**(KEY_SIZE-1),(2**KEY_SIZE)-1)
        N2 = p2*q2
    ...
    def ap_assoc(self, ap_data, body):
        if not "envelope" in body:
            return (False)
        pi = sympy.randprime(2**(KEY_SIZE-1),(2**KEY_SIZE)-1)
        qi = sympy.randprime(2**(KEY_SIZE-1),(2**KEY_SIZE)-1)
        Ni = pi*qi
    ...
    def client_assoc(self):
        p1 = sympy.randprime(2**(KEY_SIZE-1),(2**KEY_SIZE)-1)
        q1 = sympy.randprime(2**(KEY_SIZE-1),(2**KEY_SIZE)-1)
        N1 = p1*q1
